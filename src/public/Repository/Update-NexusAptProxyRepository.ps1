function Update-NexusAptProxyRepository {
    <#
.SYNOPSIS
...
Location of the remote repository being proxied, e.g. https://api.Apt.org/v3/index.json
...
Update-NexusAptProxyRepository @RepoParams
# Updates the specified Apt proxy repository with the provided parameters
#>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusAptProxyRepository/', DefaultParameterSetname = "Default", SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/apt/proxy/$Name"

        $Body = Get-NexusRepositorySettings -Format apt -Name $Name -Type proxy -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        switch -Wildcard ($PSBoundParameters.Keys) {
            ...
            "FlatUpstream" {
                if ($Body.apt.flat -ne $FlatUpstream.IsPresent) {
                    $Body.apt.flat = $FlatUpstream.IsPresent
                    $Modified = $true
                }
            }
        }
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update Apt Proxy Repository")) {
