function Update-NexusRHostedRepository {
    <#
    .SYNOPSIS
    Updates properties of a given R hosted repository

    .PARAMETER Name
    The name of the R hosted repository to update
    ...
    .EXAMPLE
    Update-NexusRHostedRepository -Name MyRRepo -BlobStoreName default

    # Updates the specified R hosted repository with the provided parameters
    #>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusRHostedRepository/', SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/r/hosted/$Name"

        $Body = Get-NexusRepositorySettings -Format r -Name $Name -Type hosted -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update R Hosted Repository")) {
