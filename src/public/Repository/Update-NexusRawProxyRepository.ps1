function Update-NexusRawProxyRepository {
    <#
.SYNOPSIS
Creates a new Raw Proxy Repository
...
Location of the remote repository being proxied, e.g. https://api.Raw.org/v3/index.json
...
Update-NexusRawProxyRepository @ProxyParameters
# Updates the specified Raw proxy repository with the provided parameters
#>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusRawProxyRepository/', DefaultParameterSetname = "Default", SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/raw/proxy/$Name"

        $Body = Get-NexusRepositorySettings -Format raw -Name $Name -Type proxy -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update Raw Proxy Repository")) {
