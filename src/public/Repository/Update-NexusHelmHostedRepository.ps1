function Update-NexusHelmHostedRepository {
    <#
    .SYNOPSIS
    Updates properties of a given Helm hosted repository

    .PARAMETER Name
    The name of the Helm hosted repository to update
    ...
    .EXAMPLE
    Update-NexusHelmHostedRepository -Name MyHelmRepo -BlobStoreName default

    # Updates the specified Helm hosted repository with the provided parameters
    #>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusHelmHostedRepository/', SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/helm/hosted/$Name"

        $Body = Get-NexusRepositorySettings -Format helm -Name $Name -Type hosted -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update Helm Hosted Repository")) {
