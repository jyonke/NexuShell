function Update-NexusHelmProxyRepository {
    <#
.SYNOPSIS
Creates a new Helm Proxy Repository
...
Location of the remote repository being proxied, e.g. https://api.Helm.org/v3/index.json
...
$ProxyParameters = @{
    Name = 'HelmProxy'
    ProxyRemoteUrl = 'https://example.com/charts'
    CleanupPolicy = '90_Days'
    UseNegativeCache = $true
}

Update-NexusHelmProxyRepository @ProxyParameters
# Updates the specified Helm proxy repository with the provided parameters
#>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusHelmProxyRepository/', DefaultParameterSetname = "Default", SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/helm/proxy/$Name"

        $Body = Get-NexusRepositorySettings -Format helm -Name $Name -Type proxy -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update Helm Proxy Repository")) {
