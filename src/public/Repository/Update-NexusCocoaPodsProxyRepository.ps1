function Update-NexusCocoaPodsProxyRepository {
    <#
.SYNOPSIS
...
Location of the remote repository being proxied, e.g. https://api.CocoaPods.org
...
$ProxyParameters = @{
    Name = 'CocoaPodsProxy'
    ProxyRemoteUrl = 'https://repo.CocoaPods.local'
    CleanupPolicy = '90_Days'
    UseNegativeCache = $true
}

Update-NexusCocoaPodsProxyRepository @ProxyParameters
# Updates the specified CocoaPods proxy repository with the provided parameters
#>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusCocoaPodsProxyRepository/', DefaultParameterSetname = "Default", SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/cocoapods/proxy/$Name"

        $Body = Get-NexusRepositorySettings -Format cocoapods -Name $Name -Type proxy -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update CocoaPods Proxy Repository")) {
