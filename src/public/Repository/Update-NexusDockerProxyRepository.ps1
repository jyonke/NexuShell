function Update-NexusDockerProxyRepository {
    <#
.SYNOPSIS
Creates a new Docker Proxy Repository
...
Location of the remote repository being proxied, e.g. https://api.Docker.org/v3/index.json
...
.EXAMPLE
$ProxyParameters = @{
    Name = 'DockerProxyTest'
    CleanupPolicy = '90_Days'
    UseNegativeCache = $true
}

Update-NexusDockerProxyRepository @ProxyParameters
# Updates the specified Docker proxy repository with the provided parameters
#>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusDockerProxyRepository/', DefaultParameterSetname = "Default", SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/docker/proxy/$Name"

        $Body = Get-NexusRepositorySettings -Format docker -Name $Name -Type proxy -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        switch -Wildcard ($PSBoundParameters.Keys) {
            ...
            'ProxyIndexType' {
                if ($Body.dockerProxy.indexType -ne $ProxyIndexType.ToUpper()) {
                    $Body.dockerProxy.indexType = $ProxyIndexType.ToUpper()
                    $Modified = $true
                }
            }
            'IndexUrl' {
                if ($Body.dockerProxy.indexUrl -ne $IndexUrl) {
                    $Body.dockerProxy.indexUrl = $IndexUrl
                    $Modified = $true
                }
            }
        }
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update Docker Proxy Repository")) {
