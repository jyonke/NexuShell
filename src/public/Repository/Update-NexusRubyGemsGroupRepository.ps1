function Update-NexusRubyGemsGroupRepository {
    <#
    .SYNOPSIS
    Updates properties of a given RubyGems group repository

    .PARAMETER Name
    The name of the RubyGems group repository to update

    .PARAMETER GroupMembers
    The RubyGems Repositories to add as group members
    ...
    .EXAMPLE
    Update-NexusRubyGemsGroupRepository -Name RubyGems-group -GroupMembers 'RubyGems-proxy','RubyGemsHostedTest'

    # Updates the specified RubyGems group repository with the provided parameters
    #>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusRubyGemsGroupRepository/', SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [String]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/rubygems/group/$Name"

        $Body = Get-NexusRepositorySettings -Format rubygems -Name $Name -Type group -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update RubyGems Group Repository")) {
