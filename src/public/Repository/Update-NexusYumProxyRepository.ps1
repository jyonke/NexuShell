function Update-NexusYumProxyRepository {
    <#
.SYNOPSIS
Creates a new Yum Proxy Repository
...
Update-NexusYumProxyRepository @ProxyParameters
# Updates the specified Yum proxy repository with the provided parameters
#>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusYumProxyRepository/', DefaultParameterSetname = "Default", SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/yum/proxy/$Name"

        $Body = Get-NexusRepositorySettings -Format yum -Name $Name -Type proxy -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update Yum Proxy Repository")) {
