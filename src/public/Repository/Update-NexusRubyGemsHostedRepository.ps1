function Update-NexusRubyGemsHostedRepository {
    <#
    .SYNOPSIS
    Updates properties of a given RubyGems hosted repository

    .PARAMETER Name
    The name of the RubyGems hosted repository to update
    ...
    .EXAMPLE
    Update-NexusRubyGemsHostedRepository -Name RubyGemsHostedTest -BlobStoreName default

    # Updates the specified RubyGems hosted repository with the provided parameters
    #>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusRubyGemsHostedRepository/', SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/rubygems/hosted/$Name"

        $Body = Get-NexusRepositorySettings -Format rubygems -Name $Name -Type hosted -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update RubyGems Hosted Repository")) {
