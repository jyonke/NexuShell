function Update-NexusYumGroupRepository {
    <#
    .SYNOPSIS
    Updates properties of a given Yum group repository

    .PARAMETER Name
    The name of the Yum group repository to update
    ...
    The Yum Repositories to add as group members
    ...
    .EXAMPLE
    Update-NexusYumGroupRepository -Name Yum-group -GroupMembers 'YumProxy','MyYumRepo'

    # Updates the specified Yum group repository with the provided parameters
    #>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusYumGroupRepository/', SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [String]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/yum/group/$Name"

        $Body = Get-NexusRepositorySettings -Format yum -Name $Name -Type group -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable

        if ($SigningKey -or $SigningKeyPassphrase) {
            $Body.yumSigning = @{}        
        }
        ...
        switch -Wildcard ($PSBoundParameters.Keys) {
            ...
            'SigningKey' {
                $Body.yumSigning.keypair = $SigningKey
                $Modified = $true
            }
            'SigningKeyPassphrase' {
                $Body.yumSigning.passphrase = $SigningKeyPassphrase
                $Modified = $true
            }
        }

        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update Yum Group Repository")) {
