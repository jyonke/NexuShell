function Update-NexusP2ProxyRepository {
    <#
.SYNOPSIS
...
Update-NexusP2ProxyRepository @ProxyParameters
# Updates the specified P2 proxy repository with the provided parameters
#>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusP2ProxyRepository/', DefaultParameterSetname = "Default", SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/p2/proxy/$Name"

        $Body = Get-NexusRepositorySettings -Format p2 -Name $Name -Type proxy -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update P2 Proxy Repository")) {
