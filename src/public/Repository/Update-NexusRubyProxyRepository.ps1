function Update-NexusRubyGemsProxyRepository {
    <#
.SYNOPSIS
Creates a new RubyGems Proxy Repository
...
Location of the remote repository being proxied, e.g. https://api.RubyGems.org/v3/index.json
...
$ProxyParameters = @{
    Name = 'RubyGemsProxy'
    ProxyRemoteUrl = 'https://example.org'
    CleanupPolicy = '90_Days'
    UseNegativeCache = $true
}

Update-NexusRubyGemsProxyRepository @ProxyParameters
# Updates the specified RubyGems proxy repository with the provided parameters
#>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusRubyGemsProxyRepository/', DefaultParameterSetname = "Default", SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/rubygems/proxy/$Name"

        $Body = Get-NexusRepositorySettings -Format rubygems -Name $Name -Type proxy -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update RubyGems Proxy Repository")) {
