function Update-NexusGitLfsHostedRepository {
    <#
    .SYNOPSIS
    Updates properties of a given GitLfs hosted repository

    .PARAMETER Name
    The name of the GitLfs hosted repository to update
    ...
    .EXAMPLE
    Update-NexusGitLfsHostedRepository -Name MyGitLfsRepo -BlobStoreName default

    # Updates the specified GitLfs hosted repository with the provided parameters
    #>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusGitLfsHostedRepository/', SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/gitlfs/hosted/$Name"

        $Body = Get-NexusRepositorySettings -Format gitlfs -Name $Name -Type hosted -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update GitLfs Hosted Repository")) {
