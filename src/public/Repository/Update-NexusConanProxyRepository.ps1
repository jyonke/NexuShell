function Update-NexusConanProxyRepository {
    <#
.SYNOPSIS
...
Location of the remote repository being proxied, e.g. https://api.Conan.org
...
$ProxyParameters = @{
    Name = 'ConanProxy'
    ProxyRemoteUrl = 'https://repo.conan.org/api/v2'
    CleanupPolicy = '90_Days'
    UseNegativeCache = $true
}

Update-NexusConanProxyRepository @ProxyParameters
# Updates the specified Conan proxy repository with the provided parameters
#>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusConanProxyRepository/', DefaultParameterSetname = "Default", SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/conan/proxy/$Name"

        $Body = Get-NexusRepositorySettings -Format conan -Name $Name -Type proxy -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update Conan Proxy Repository")) {
