function Update-NexusPyPiProxyRepository {
    <#
.SYNOPSIS
Creates a new PyPi Proxy Repository
...
Location of the remote repository being proxied, e.g. https://api.PyPi.org/v3/index.json
...
$ProxyParameters = @{
    Name = 'PyPiProxy'
    ProxyRemoteUrl = 'https://pypi.org'
    CleanupPolicy = '90_Days'
    UseNegativeCache = $true
}

Update-NexusPyPiProxyRepository @ProxyParameters
# Updates the specified PyPi proxy repository with the provided parameters
#>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusPyPiProxyRepository/', DefaultParameterSetname = "Default", SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/pypi/proxy/$Name"

        $Body = Get-NexusRepositorySettings -Format pypi -Name $Name -Type proxy -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update PyPi Proxy Repository")) {
