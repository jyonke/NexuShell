function Update-NexusCondaProxyRepository {
    <#
.SYNOPSIS
...
$ProxyParameters = @{
    Name = 'CondaProxy'
    ProxyRemoteUrl = 'https://conda.org/api/v2'
    CleanupPolicy = '90_Days'
    UseNegativeCache = $true
}

Update-NexusCondaProxyRepository @ProxyParameters
# Updates the specified Conda proxy repository with the provided parameters
#>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusCondaProxyRepository/', DefaultParameterSetname = "Default", SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/conda/proxy/$Name"

        $Body = Get-NexusRepositorySettings -Format conda -Name $Name -Type proxy -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update Conda Proxy Repository")) {
