function Update-NexusNPMProxyRepository {
    <#
.SYNOPSIS
Creates a new NPM Proxy Repository
...
Location of the remote repository being proxied, e.g. https://api.NPM.org/v3/index.json
...
$ProxyParameters = @{
    Name = 'NPMProxy'
    ProxyRemoteUrl = 'https://example.com/NPM'
    CleanupPolicy = '90_Days'
    UseNegativeCache = $true
}

Update-NexusNPMProxyRepository @ProxyParameters
# Updates the specified NPM proxy repository with the provided parameters
#>
    [CmdletBinding(HelpUri = 'https://nexushell.dev/Update-NexusNPMProxyRepository/', DefaultParameterSetname = "Default", SupportsShouldProcess, ConfirmImpact = 'High')]
    param(
        [Parameter(Mandatory)]
        [string]
        $Name,
    ...
    begin {
        ...
        $urislug = "/service/rest/v1/repositories/npm/proxy/$Name"

        $Body = Get-NexusRepositorySettings -Format npm -Name $Name -Type proxy -ErrorAction 'Stop' | Select-Object -Property * -ExcludeProperty format, type | Convert-ObjectToHashtable
        ...
        if ($Modified) {
            if ($PSCmdlet.ShouldProcess($Name, "Update NPM Proxy Repository")) {
